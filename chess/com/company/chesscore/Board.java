...
public class Board {
    ...
    public String createMoveString(int myRow, int myCol, int nextRow, int nextCol) {
        ...
        return Integer.toString(getIntPosition(nextRow, nextCol));
    }
    ...
    public Piece movePiece(int fromPosition, int toPosition) {// returns captured piece , needs to be captured by ft
            ...
            for (int j = -1; j <= 1; j += 2)// i and j for "direction"
            {
                try {
                    ...
                    if ((getBoardSquare(getIntPosition(newR, newC)).getPiece() instanceof Bishop
                            ...
                            getBoardSquare(getIntPosition(newR, newC)).getPiece().getColorNum() != myColor) {// System.out.println("queen
                                                                                                             // or
                                                                                                             // bishop
                                                                                                             // strikes");
                        return false;
            ...
            for (int j = -1; j <= 1; j++)// i and j for "direction"
            {
                ...
                try {
                    ...
                    if ((getBoardSquare(getIntPosition(newR, newC)).getPiece() instanceof Rook
                            || getBoardSquare(getIntPosition(newR, newC)).getPiece() instanceof Queen) &&
                            getBoardSquare(getIntPosition(newR, newC)).getPiece().getColorNum() != myColor) {// System.out.println("rook
                                                                                                             // or
                                                                                                             // bishop
                                                                                                             // strikes");
                        return false;
                ...
                for (int j = -1; j <= 1; j += 2) {
                    ...
                    try {// try catch lazem in loop cuz if out of lop it ends
                        ...
                        if (getBoardSquare(getIntPosition(newR, newC)).getPiece() instanceof Knight &&
                                getBoardSquare(getIntPosition(newR, newC)).getPiece().getColorNum() != myColor) {// System.out.println("knight
                                                                                                                 // strikes");
                            return false;
                        }
                    } catch (IllegalArgumentException e) {
        ...
        for (int i = -1; i <= 1; i += 2) {
            try {
                ...
                if (getBoardSquare(getIntPosition(newR, newC)).getPiece() instanceof Pawn &&
                        getBoardSquare(getIntPosition(newR, newC)).getPiece().getColorNum() != myColor) {// System.out.println("pawn
                                                                                                         // strikes");
                    return false;
                }
            } catch (IllegalArgumentException e) {
                ...
                if (i != 0 || j != 0)// false only when both 0
                    try {
                        ...
                        if (getBoardSquare(getIntPosition(newR, newC)).getPiece() instanceof King &&
                                getBoardSquare(getIntPosition(newR, newC)).getPiece().getColorNum() != myColor) {// System.out.println("king
                                                                                                                 // strikes");
                            return false;
                        }
                    } catch (IllegalArgumentException e) {
                    ...
                    if (myPosition == 60 && nextPosition == 63) {
                        ...
                        if (kingSquare.getPiece().firstMove && rookSquare.getPiece().firstMove) {
                            ...
                            if (getBoardSquare(61).getPiece().getColor().equals("Empty Square")
                                    && getBoardSquare(62).getPiece().getColor().equals("Empty Square")
                                    && whiteRookRightCastle(myPosition)) {
                        ...
                        if (kingSquare.getPiece().firstMove && rookSquare.getPiece().firstMove) {
                            if (getBoardSquare(59).getPiece().getColor().equals("Empty Square")
                                    ...
                                    && getBoardSquare(57).getPiece().getColor().equals("Empty Square")
                                    && whiteRookLeftCastle(myPosition))
                                return true;
                        }
                    }
                } else {
                    if (myPosition == 4 && nextPosition == 7) {
                        if (kingSquare.getPiece().firstMove && rookSquare.getPiece().firstMove) {
                            if (getBoardSquare(5).getPiece().getColor().equals("Empty Square")
                                    && getBoardSquare(6).getPiece().getColor().equals("Empty Square")
                                    && blackRookRightCastle(myPosition)) {
                                return true;
                            }
                        }
                        ...
                        if (kingSquare.getPiece().firstMove && rookSquare.getPiece().firstMove) {
                            if (getBoardSquare(3).getPiece().getColor().equals("Empty Square")
                                    ...
                                    && getBoardSquare(1).getPiece().getColor().equals("Empty Square")
                                    && blackRookLeftCastle(myPosition))
                                return true;
                        }
                    }
                }
